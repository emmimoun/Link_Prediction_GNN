            ...
            for j in range(0,len(to_neighs[i])):
                if(time_neighs[i][j]<=self.time):
                    if(to_neighs[i][j] not in dic_edge[i]):
                        dic_edge[i][to_neighs[i][j]]=(math.pi*100) *math.exp(-0.5*((time_neighs[i][j]-self.time)/100)**2)
                    else:
                        dic_edge[i][to_neighs[i][j]]+=(math.pi*100) *math.exp(-0.5*((time_neighs[i][j]-self.time)/100)**2)
                    total_value[i]+=float((math.pi*100) *math.exp(-0.5*((time_neighs[i][j]-self.time)/100)**2))
